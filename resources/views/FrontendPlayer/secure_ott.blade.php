...
<head>
  ...
  <style>
    :root{
      --bg:#0b0b0b; --bg-soft:#121212; --text:#ffffff; --muted:#b5b5b5;
      --accent:#ff004c; --accent-2:#ff7b00; --card:#1a1a1a;
    }
    *{box-sizing:border-box}
    ...
    /* Navbar */
    .navbar{position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;padding:14px 20px;background:rgba(0,0,0,.7);backdrop-filter:blur(8px);border-bottom:1px solid rgba(255,255,255,.06)}
    .logo{font-weight:800;font-size:1.25rem;background:linear-gradient(45deg,var(--accent),var(--accent-2));-webkit-background-clip:text;color:transparent}
    .nav-links{display:flex;gap:18px;align-items:center}
    .nav-links a{color:#e9e9e9;opacity:.9}
    .nav-links a:hover{color:var(--accent)}
    .cta{padding:10px 14px;border:none;border-radius:10px;background:linear-gradient(45deg,var(--accent),var(--accent-2));color:#fff;font-weight:700;cursor:pointer}

    /* Player */
    .player{max-width:1280px;margin:18px auto 0;padding:0 16px}
    .player-box{position:relative;background:#000;border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,.08)}
    .player-box::after{content:"";display:block;aspect-ratio:16/9}
    .player-box video{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#000}

    /* Sections */
    .section{padding:28px 16px 8px;max-width:1280px;margin-inline:auto}
    .section h2{font-size:1.25rem;margin:0 0 10px}
    .section .muted{color:var(--muted);font-size:.95rem}

    /* Row scrolling */
    .row-wrapper{position:relative}
    .row{display:flex;gap:14px;align-items:stretch;overflow-x:auto;overflow-y:hidden;padding:10px 2px 20px;scroll-snap-type:x mandatory}
    .row::-webkit-scrollbar{height:10px}
    .row::-webkit-scrollbar-thumb{background:#2a2a2a;border-radius:999px}

    .card{min-width:180px;max-width:220px;flex:0 0 auto;background:var(--card);border:1px solid rgba(255,255,255,.06);border-radius:16px;overflow:hidden;scroll-snap-align:start;transition:transform .2s ease}
    .card:hover{transform:translateY(-4px);box-shadow:0 12px 24px rgba(0,0,0,.35)}
    .poster{position:relative;aspect-ratio:2/3;background:#222}
    .badge{position:absolute;top:8px;left:8px;padding:4px 8px;border-radius:999px;background:rgba(0,0,0,.6);border:1px solid rgba(255,255,255,.12);font-size:.75rem}
    .meta{padding:10px 12px}
    .title{font-weight:700;line-height:1.2;margin-bottom:6px;font-size:.98rem}
    .sub{color:var(--muted);font-size:.85rem}
    .actions{display:flex;gap:8px;margin-top:12px}
    .play,.add{flex:1;padding:8px 10px;border-radius:10px;border:none;cursor:pointer;font-weight:700}
    .play{background:linear-gradient(45deg,var(--accent),var(--accent-2));color:#fff}
    .add{background:transparent;border:1px solid rgba(255,255,255,.16);color:#fff}

    /* Row arrows */
    .arrow{position:absolute;top:50%;transform:translateY(-50%);width:44px;height:44px;border-radius:50%;border:1px solid rgba(255,255,255,.18);background:rgba(0,0,0,.45);backdrop-filter:blur(6px);display:grid;place-items:center;cursor:pointer;z-index:2}
    .arrow.left{left:-4px}
    .arrow.right{right:-4px}

    /* Series Panel */
    #spSeasons li button{width:100%;text-align:left;background:transparent;color:#fff;border:1px solid rgba(255,255,255,.12);padding:8px 10px;border-radius:8px;margin:8px;cursor:pointer}
    #spSeasons li button.active{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.24)}
    #spEpisodes .ep-card{border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;background:#0f0f0f}
    #spEpisodes .ep-title{font-weight:700;margin-bottom:6px}
    #spEpisodes .ep-meta{color:#b5b5b5;font-size:.9rem;margin-bottom:8px}
    #spEpisodes .ep-actions button{background:linear-gradient(45deg,#ff004c,#ff7b00);border:none;color:#fff;border-radius:8px;padding:6px 10px;cursor:pointer}

    /* Purchase Modal */
    .modal{position:fixed;inset:0;background:rgba(0,0,0,.8);display:none;align-items:center;justify-content:center;z-index:100}
    .modal-content{background:#111;border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:20px;max-width:400px;text-align:center}
    .modal-title{font-size:1.2rem;margin-bottom:12px;color:var(--accent)}
    .modal-text{color:var(--muted);margin-bottom:20px}
    .modal-actions{display:flex;gap:12px;justify-content:center}
    .modal-actions button{padding:10px 16px;border-radius:8px;border:none;cursor:pointer;font-weight:600}
    .btn-primary{background:linear-gradient(45deg,var(--accent),var(--accent-2));color:#fff}
    .btn-secondary{background:transparent;border:1px solid rgba(255,255,255,.16);color:#fff}
...
<body>
  ...
  <section class="player">
    <div class="player-box">
      <video id="videoPlayer" controls playsinline controlsList="nodownload noplaybackrate" disablePictureInPicture oncontextmenu="return false" poster="{{ $poster ?? '' }}">
        <!-- Secure HLS playback -->
      </video>
      <div id="wm" style="position:absolute;inset:auto 8px 8px auto;z-index:2;color:rgba(255,255,255,.65);font-size:12px;pointer-events:none;">{{ auth()->user()->email ?? 'Guest' }} • <span id="timestamp"></span></div>
    </div>
  </section>
  ...
  <section id="seriesPanel" style="display:none;max-width:1280px;margin:12px auto 0;padding:0 16px">
    <div style="background:#111;border:1px solid rgba(255,255,255,.08);border-radius:12px;overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.06)">
        ...
        <button id="spClose" style="background:transparent;border:1px solid rgba(255,255,255,.16);color:#fff;border-radius:8px;padding:6px 10px;cursor:pointer">Close</button>
      </div>
      <div style="display:flex;gap:0;min-height:220px">
        ...
        <main style="flex:1;padding:12px;max-height:360px;overflow:auto">
          <div id="spEpisodes" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px"></div>
        </main>
      </div>
    </div>
  </section>
      ...
      try {
        ...
        if (videoUrl) {
          if (isWebseries && mainData.seasonsId && mainData.seasonsId.length > 0) {
            ...
            const secureUrl = `/api/video-manifest?url=${encodeURIComponent(videoUrl)}&token=${btoa(JSON.stringify({timestamp: Math.floor(Date.now()/1000)}))}`;
    ...
    async function playSecureUrl(url, title = '', contentId = '', contentType = 'movie', channelId = ''){
      ...
      try {
        ...
        if (contentType === 'webseries') {
          ...
          checkParams = { id: contentId, user_id: USER_CONFIG.id, channel_id: channelId, field: '_id' };
        } else {
          checkUrl = '/api/ott/check-movie';
          checkParams = { vId: contentId, user_id: USER_CONFIG.id, channel_id: channelId, country: USER_CONFIG.country };
        }
        ...
        // Play video securely
        const secureUrl = `/api/video-manifest?url=${encodeURIComponent(url)}&token=${btoa(JSON.stringify({timestamp: Math.floor(Date.now()/1000)}))}`;
        ...
        if (video.canPlayType('application/vnd.apple.mpegurl')) {
          ...
          hlsInstance = new Hls({ maxBufferLength: 60, backBufferLength: 60 });
      ...
      function selectSeason(seasonData, btnEl, seriesData){
        ...
        eps.forEach((ep, i) => {
          ...
          card.querySelector('button').addEventListener('click', ()=> {
            if (m3u8) {
              playSecureUrl(m3u8, ep.title || `Episode ${i+1}`, ep._id || ep.id, 'webseries', seriesData.channel_id);
    ...
    (async function(){
      try {
        ...
        function createVideoCard(videoItem) {
          const poster = videoItem.thumbnail_url || videoItem.poster_url || videoItem.image_url || `https://picsum.photos/400/600?random=${Math.floor(Math.random()*1000)}`;
          const title = videoItem.title || 'Untitled';
          const isWebseries = String(videoItem.is_tvseries) === '1';
