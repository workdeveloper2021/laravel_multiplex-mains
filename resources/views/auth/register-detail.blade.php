...
<div class="flex flex-1 justify-center items-center px-4">
    <div class="bg-white p-8 md:p-10 rounded-xl shadow-xl w-full max-w-md">
        ...
        <h2 class="text-2xl font-bold text-orange-600 text-center mb-6 flex items-center justify-center gap-2">
            <i class="fas fa-user text-orange-500"></i> Register Details
        </h2>
        <form action="{{ route('register.detail.save') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <!-- Channel email -->
            <div>
                <input type="text" name="email" placeholder="Channel Email"
                    class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none" value="{{ old('email') }}" />
            </div>
            <!-- Channel Password -->
            <div>
                <input type="password" name="password" placeholder="Channel Password"
                    class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none" value="{{ old('password') }}" />
            </div>
